...
| (4) | $ 08 ~ $ 0A | 音の大きさをコントロールする。  音量固定方式（プログラム可能）、音量可変方式を選択可能 |
...
| (11) | $ 2D ~ $ 2F | 入力クロックの分周比を指定  FM音源は1/2 ~ 1/6、矩形波音源は1 ~ 1/4 |
| (12) | $ 30 ~ $ 3E | DetuneとMultipleをコントロール。</br>音色の設定に使用し、基本波と倍音の関係をコントロールします。 |
| (13) | $ 40 ~ $ 4E | 合計レベルを指定する。</br>この情報が、音量の変調指数、変調波の変調指数となります。 |
| (14) | $ 50 ~ $ 5E | キーボード入力に応じて、A・D・S・Rの変化率を調整する。  アタックレートは、アタック時のエンベロープの変化率です。 |
| (15) | $ 60 ~ $ 6E | Decay Rateの設定。減衰時のエンベロープの変化率を指定します。 |
| (16) | $ 70 ~ $ 7E | Sustain Rateの設定。Sustain時のエンベロープのレート変化量を指定します。 |
| (17) | $ 80 ~ $ 8E | Sustain levelの設定。減衰からSustainへの移行の度合いを指定します。  リリースレートは、リリース時のエンベロープの変化率を示します。 |
| (18) | $ 90 ~ $ 9E | 矩形波音源のようなリピートパターンを含むエンベロープを生成します。 |
| (19) | $ A0 ~ $ A6 | 各チャンネルのキーコード（F番号）を付与する。 |
| (20) | $ A8 ~ $ AE | 特定のモードに設定した時の3チャンネルのキーコード（F番号）を付与する。 |
| (21) | $ B0 ~ $ B2 | FM変調の変調方式（接続）とフィードバックFM（自己帰還型）の変調係数を与える。 |

☆FM方式について  
　FM方式では、周波数変調を利用して様々な高次高調波を制御することにより、楽音を合成しています。  
　FM方式の基本式は以下の通りです。

<div align="center">F = A sin(ωC t + I sin(ωM t)) </div>
<div align="right">- (1)</div>

ここで、A は出力振幅、I は変調指数、ωC と ωM はそれぞれ搬送波と変調器の角周波数です。  
この式は、次のように表すこともできます。

<div align="center">F = A [J<sub>0</sub> sin(ωC t) + J<sub>1</sub> (I) (sin(ωC + ωM) t - sin(ωC -ωM)t)</div>
<div align="center">　　　　　　　　　+ J<sub>2</sub> (I) (sin(ωC + 2ωM) t - sin(ωC - 2ωM)t)</div>
<div align="center">+ ・・・・・・]</div>
<div align="right">- (2)</div>

ここで、Jn (I) はn番目の第一種ベッセル関数です。上式に示すように、FMシステムには様々な高調波が含まれており、それらを制御することができます。上式に示すように、FMシステムには様々な高調波が含まれており、それらを制御することができます。
OPNでは、上記のFM変調に加え、（3）、（4）に示す多重TM変調とフィードバックFM変調を行い、あらゆる音を出すことが可能です。

<div align="center">F = A sin(ωM<sub>1</sub>t + I<sub>2</sub> sin(ωM<sub>2</sub>t))</div>
<div align="right">- (3)</div>

<div align="center">F = A sin(ωC t + βF)</div>
